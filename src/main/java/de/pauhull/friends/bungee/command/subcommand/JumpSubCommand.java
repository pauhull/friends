...
public class JumpSubCommand extends SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getPrefix() + "§c/friend jump <Spieler>"));
            return;
        }
                            ...
                            friends.getSettingsTable().getJumping(requestedUUID, jumping -> {
                                if (jumping) {
                                    if (requester.getServer().getInfo().equals(requested.getServer().getInfo())) {
                                        requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getPrefix() + friends.getMessages().getSameServer()));
                                    } else {
                                        requester.connect(requested.getServer().getInfo());
                                    }
                                } else {
                                    requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getPrefix() + String.format(friends.getMessages().getNoJumping(), requested.getName())));
                                }
                            });
                        } else {
                            friends.getUuidFetcher().fetchNameAsync(requestedUUID, name -> {
                                requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getPrefix() + String.format(friends.getMessages().getNotOnline(), name)));
                            });
                        }
                    } else {
                        friends.getUuidFetcher().fetchNameAsync(requestedUUID, name -> {
                            requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getPrefix() + String.format(friends.getMessages().getNoFriend(), name)));
                        });
                    }
